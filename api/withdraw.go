...
func Withdraw(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		res.Message = fmt.Sprintf("Could not transfer funds with broker id: %d, debit account id: %d, currency id: %d", w.BrokerId, w.AccountId, w.CurrencyId)
		res.Code = 0
		return c.JSON(res)
